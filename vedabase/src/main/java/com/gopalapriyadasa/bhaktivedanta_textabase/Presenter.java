...
/**
 * Created by Gopa702 on 8/30/2017.
 */
public class Presenter implements LoaderManager.LoaderCallbacks<List<File>> {
    ...
    private void openFile(File file) {
        ...
        if (fullPath.endsWith(".ivd")) { //we have determined a mime type and can probably handle the file.
            try {
                /*Implicit intent representing the action we want.  The system will determine is it
                can handle the request.*/
                Intent i = new Intent();
                i.putExtra("filePath", fullPath);
                mView.getActivity().setResult(Activity.RESULT_OK, i);
